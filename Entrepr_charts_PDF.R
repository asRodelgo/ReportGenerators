...
radar_chart <- function(couName){      
  ...
  # filter the data
  data <- filter(Entrepr_data, CountryCode %in% c(cou,neighbors), Section == "Markets", Subsection=="radar1")
...
## ---- combo_percent ----
combo_percent <- function(couName){      
  
  cou <- .getCountryCode(couName)
  
  data <- filter(Entrepr_data, CountryCode==cou, Section == "Markets", Subsection=="combo1")
...
## ---- table_time_avg ----
table_time_avg <- function(couName,table){      
  
  cou <- .getCountryCode(couName)
  table <- "table1"
  tableKeys <- unique(filter(Entrepr_data, Section == "Policy", Subsection==table)[,c("Key","IndicatorShort")])
  data <- filter(Entrepr_data, CountryCode==cou, Section == "Policy", Subsection==table)
...
table_time_avg(couName,"table1")

## ---- sparklines ----
sparklines <- function(couName,table){      
...
sparklines(couName,"table1")

## ---- table_region ----
table_region <- function(couName){      
  
  cou <- .getCountryCode(couName)
  data <- filter(Entrepr_data, CountryCode==cou, Section == "Policy", Subsection=="table1") #select country, region and world
  if (nrow(data[data$CountryCode==cou,])>0){
    
    couRegion <- as.character(countries[countries$CountryCodeISO3==cou,]$RegionCodeES)  # obtain the region for the selected country
    data <- filter(Entrepr_data, CountryCode %in% c(cou,couRegion, "RWe"), Section == "Policy", Subsection=="table1") #select country, region and world
    ...
    # keep the latest period (excluding projections further than 2 years)
    data <- filter(data, Period == max(as.numeric(Period),na.rm = TRUE))
    #data <- filter(data, Period <= (as.numeric(thisYear) + 1))
    # remove NAs rows
    data <- filter(data, !is.na(Observation))
...
table_region(couName)

## ---- table_time ----
table_time <- function(couName,table){      
